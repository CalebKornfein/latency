...
def binary_metrics(y_true, y_pred, paramdict=None, title=None, label=None, output_text=False, filepath="/metric_outputs/", binary=False, plot=True):
    ...
    if binary:
        ...
        f1 = f1_score(y_true, np.array(
            [y_pred[j] >= 0.5 for j in range(len(y_pred))], dtype="int"))
    ...
    if plot:
        ...
        plt.plot(fpr, tpr, linestyle="--", marker=".", markersize=15,
                 label="{} AUC: {:0.4}, F1: {:0.4}".format(label, aucs, f1))
...
def overlay_roc(clf, X_train, X_test, y_train, y_test, title=None):
    ...
    plt.plot(train_fpr, train_tpr, linestyle="--", marker=".",
             markersize=15, label=f'Train, AUC = {round(train_aucs, 3)}')
    plt.plot(test_fpr, test_tpr, linestyle="--", marker=".",
             markersize=15, label=f'Test, AUC = {round(test_aucs, 3)}')
    plt.plot([0, 1], [0, 1], linestyle="--", c="k")
    plt.legend()
...
def overlay_pr(clf, X_train, X_test, y_train, y_test, title=None):
    ...
    plt.plot(train_re, train_pr, linestyle="--", marker=".",
             markersize=15, label=f'Train, AUC = {round(train_auc, 3)}')
    plt.plot(test_re, test_pr, linestyle="--", marker=".",
             markersize=15, label=f'Test, AUC = {round(test_auc, 3)}')
    plt.plot([0, 1], [0, 1], linestyle="--", c="k")
    plt.legend()
...
def overlay_pr_temp(clf, X_train, X_test, y_train, y_test, X_new_train, y_new_test, title=None):
    ...
    test_pr, test_re, test_thresholds = precision_recall_curve(
        y_test, y_test_pred)
    new_test_pr, new_test_re, new_test_thresholds = precision_recall_curve(
        y_new_test, y_new_test_pred)
    ...
    plt.plot(test_re, test_pr, linestyle="--", marker=".",
             markersize=15, label=f'Test, AUC = {round(test_auc, 3)}')
    plt.plot(new_test_re, new_test_pr, linestyle="--", marker=".",
             markersize=15, label=f'Test on New Data, AUC = {round(new_test_auc, 3)}')
    plt.plot([0, 1], [0, 1], linestyle="--", c="k")
    plt.legend()
...
def multiclass_plot(clf, X,  y):
    ...
    for i in range(len(classes)):
        ...
        plt.plot(fpr, tpr, linestyle="--", marker=".",
                 markersize=15, label=f"{i+1} AUC: {round(aucs, 2)}")
