...
def fit_xgb(X_train, X_test, y_train, y_test, params=None):
    if params:
        clf = xgb.XGBClassifier(**params)
    else:
        clf = xgb.XGBClassifier(objective='binary:logistic',
                                max_depth=2, colsample_bytree=0.5, scale_pos_weight=10)
    clf.fit(X_train, y_train)

    overlay_roc(clf, X_train, X_test, y_train, y_test)
    overlay_pr(clf, X_train, X_test, y_train, y_test)
    plt.show()
...
def fit_rf_classifier(X_features, X_train, X_test, y_train, y_test, X_new_train, y_new_test):
    ...
    clf = RandomForestClassifier(
        class_weight='balanced', max_depth=2, random_state=0, verbose=False)
    clf.fit(X_train, y_train)
    ...
    features, importances = np.array(features)[
        sorted_idx][n_features - 10:], clf.feature_importances_[sorted_idx][n_features - 10:]
    plt.figure()
    plt.barh(features, importances)
    plt.title("Top 10 features by Gini importance:")
    overlay_pr_temp(clf, X_train, X_test, y_train,
                    y_test, X_new_train, y_new_test)
    overlay_roc_temp(clf, X_train, X_test, y_train,
                     y_test, X_new_train, y_new_test)
...
def rf_regression(X_features, X_train, X_test, y_train, y_test):
    ...
    clf = RandomForestRegressor(
        bootstrap=False, max_depth=2, random_state=0, verbose=False)
    clf.fit(X_train, y_train)
    ...
    # top ten features by importance
    features, importances = np.array(X_features)[
        sorted_idx][n_features - 10:], clf.feature_importances_[sorted_idx][n_features - 10:]
    plt.figure()
    plt.barh(features, importances)
    plt.title("Top 10 features by Gini importance:")
...
def main():
    X_train, X_test, y_train, y_test, X_new_train, y_new_test = load_data(
        label_type='HIV_Top_25')
    params = {'objective': 'binary:logistic',
              'max_depth': 2,
              'colsample_bytree': 0.5,
              'scale_pos_weight': 2}
    fit_xgb(X_train, X_test, y_train, y_test,
            X_new_train, y_new_test, params=params)
    ...
    y_hat_train = pd.DataFrame(clf.predict_proba(
        X_train)[:, 1], columns=['HIV_Binary'])
    y_hat_test = pd.DataFrame(clf.predict_proba(
        X_test)[:, 1], columns=['HIV_Binary'])

    quantile = y_train.value_counts()[0] / y_train.shape[0]
    train_predicted_zero_index, test_predicted_zero_index = threshold(
        quantile, y_hat_train, y_hat_test)
    train_predicted_zero_index, test_predicted_zero_index = threshold(
        .01, y_hat_train, y_hat_test)
