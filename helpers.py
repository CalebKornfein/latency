...
def load_data(n = 30, version = 'combined', label_type='10'):
    X_train = pd.read_csv(f"data/processed/{version}/overall_train.csv", index_col=[0])
    X_train = X_train[X_train.columns[:n]]
    X_test = pd.read_csv(f"data/processed/{version}/overall_test.csv", index_col=[0])
    X_test = X_test[X_test.columns[:n]]

    y_train = pd.read_csv(f"data/processed/{version}/HIV_train.csv", index_col = [0])[label_type]
    y_test = pd.read_csv(f"data/processed/{version}/HIV_test.csv", index_col = [0])[label_type]
...
def fetch_observation_type(X_train, X_test, y_train, y_test, type):
    observation_dict = {'1' : 'DMSO',
                        ...
                        '3' : 'Prostratin',
                        '4' : 'SAHA'}
    link_observations = [observation_dict[key[-1]] for key in json.load(open('data/processed/hiv.json', 'r'))['HIV_Float'].keys()]
