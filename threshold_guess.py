...
def get_thresholds(X, y, n_est, lr, d, max_thresholds, weight=None, backselect=True):
    ...
    # keep backselecting until the condition is satisfied.
    outp = 1
    Xp = X_new.copy()
    clfp = clf1
    itr=0
    if backselect:
        while (outp >= out1 and itr < X_new.shape[1]-1) or (itr + max_thresholds <= X_new.shape[1]):
...
# compute the thresholds
def compute_thresholds(X, y, n_est, max_depth, max_thresholds, weight=None) :
    # n_est, max_depth: GBDT parameters
    # set LR to 0.1
    lr = 0.1
    start = time.perf_counter()
    X, thresholds, header = get_thresholds(X, y, n_est, lr, max_depth, max_thresholds, weight, backselect=True)
