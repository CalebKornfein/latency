...
def create_merged_motif():
    # Observation names
    columns = from_txt('data/base/v1/rna.mat.colnames.txt', False) + from_txt('data/base/v2/rna.mat.colnames.txt', False)
...
def calculate_spearman(version, train_indices):
    ...
    if version == 'v1':
        ...
        train_indices = [index - 61708 for index in train_indices if index - 61708 >= 0]
    else:
...
def calculate_combined_spearman(train_indices):
    ...
    hiv = from_txt('data/base/v1/hiv.txt', numerical=True) + from_txt('data/base/v2/hiv.txt', numerical=True)
    hiv_train = [hiv[x] for x in train_indices]
...
def split_train_test(train_out, test_out, feature_names, version):
    ...
    test_data = []
    columns = []
    rows = from_txt(f'data/base/{version}/rna.mat.colnames.txt', numerical=False)
    ...
    for type in ['rna', 'motif']:
        ...
        with open(file, 'r') as f:
            reader = csv.reader(f)
            next(reader)
            features = from_txt(f'data/base/{version}/{type}.mat.rownames.txt', numerical=False)
            index = 0
...
if __name__ == "__main__":
    ...
    for version in ['v1', 'v2']:
        ...
        for data_type, feature_names in datasets:
            train_out = os.path.join('data', 'processed', version, data_type + "_train.csv")
            test_out = os.path.join('data', 'processed', version, data_type + "_test.csv")
