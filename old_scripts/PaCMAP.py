...
def PaCMAP(level='Observation', label_type='HIV_Binary', data='Overall'):
    if data == 'Balanced':
        X_train, X_test, y_train, y_test = load_data(
            n=10, balanced=True, label_type=label_type)
    ...
    if level == 'Observation':
        ax.scatter(X_train_PaCMAP[:, 0], X_train_PaCMAP[:, 1],
                   cmap="Spectral", c=y_train.to_numpy(), s=0.6)
    else:
        if data == 'Balanced':
            ax.scatter(X_train_PaCMAP[:, 0], X_train_PaCMAP[:, 1], s=0.6)
            for i, label in enumerate(annotations):
                ax.annotate(
                    label, (X_train_PaCMAP[i, 0], X_train_PaCMAP[i, 1]))
        else:
            ax.scatter(X_train_PaCMAP[:, 0], X_train_PaCMAP[:, 1], s=0.6)
            for i, label in enumerate(annotations):
                ax.annotate(f"{label} ({i})",
                            (X_train_PaCMAP[i, 0], X_train_PaCMAP[i, 1]))
    plt.legend()
