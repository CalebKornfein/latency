...
def config2name(path, n_features, n_est, max_depth, sample_weight, gosdt_depth, regularization, weight):
    name = '_'.join(['TG', str(n_features), str(n_est), str(max_depth), str(
        sample_weight), 'GOSDT', str(gosdt_depth), str(regularization), str(weight)]) + '.json'
...
def sweep(params, out_folder, out_csv):
    ...
    for n_est, max_depth, sample_weight in tqdm(threshold_param_combinations):
        ...
        X_train_threshold, thresholds, header, threshold_guess_time = compute_thresholds(
            X_train.copy(), y_train.copy(), n_est, max_depth, max_thresholds=30, weight=weights)
        X_test_threshold = cut(X_test.copy(), thresholds)
        X_test_threshold = X_test_threshold[header]
        print('--------------------')
        print(
            f'N_est: {n_est}, max_depth: {max_depth}, sample_weight: {sample_weight}')
        ...
        for gosdt_depth, regularization, weight in tqdm(gosdt_param_combinations):
            if weight == -1:
                ...
                X_train_threshold_ros, y_train_threshold_ros = oversample(
                    X_train_threshold.copy(), y_train.copy(), weight)
            ...
            out_path = config2name(out_folder, n_features, n_est, max_depth,
            ...
            data_row = [n_features, n_est, max_depth, sample_weight, gosdt_depth, regularization,
                        weight, train_precision, train_recall, test_precision, test_recall, model_features]
